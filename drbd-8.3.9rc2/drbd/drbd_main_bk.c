...
/*Ben.Wang*/
#define DRBD_CTR_NAME                   "drbd_ctr"
#define DRBD_CTR_MAJOR			2011
drbdraid_status drbdraid;
/*Ben.Wang*/
...
dprintk(KERN_ALERT "FALCON DEBUG: ==== drbd_send_drequest_csum\n");
	p.sector   = cpu_to_be64(sector);
...
{
dprintk(KERN_ALERT "FALCON DEBUG(_drbd_send_bio)\n");
	struct bio_vec *bvec;
	int i;
...
static int _drbd_send_zc_bio(struct drbd_conf *mdev, struct bio *bio)
{
        ...
        int leftover_len = -1;
        int base = -1;
        int jump_over_page = -1;
        char *address = NULL;

        /* hint all but last page with MSG_MORE */
        __bio_for_each_segment(bvec, bio, i, 0) {
                leftover_len = bvec->bv_len;
                base = bvec->bv_offset;
                address = page_address(bvec->bv_page);
                pg = bvec->bv_page;
                ...
                if(base >= PAGE_SIZE) 
		{
                        if (0 != base % PAGE_SIZE)
			{
                                printk(KERN_ALERT "FALCON ERROR(_drbd_send_zc_bio): ***** base % PAGE_SIZE != 0. Exit\n");
                                return 0;
                        }
                        jump_over_page = base / PAGE_SIZE;
                        pg = virt_to_page(address + jump_over_page * PAGE_SIZE);
			address += jump_over_page * PAGE_SIZE;
                        base = base - jump_over_page * PAGE_SIZE;
			//printk(KERN_ALERT "FALCON DEBUG(_drbd_send_zc_bio): jump over: %d, new base: %d\n", 
		        //							jump_over_page, base);
                }// else
                sent = PAGE_SIZE - base < leftover_len ? PAGE_SIZE - base : leftover_len;

                while (leftover_len > 0)
                {
			D_ASSERT(sent > 0);
			D_ASSERT(base >= 0);
			//printk(KERN_ALERT "FALCON DEBUG(_drbd_send_zc_bio): base: %d, sent: %d, leftover: %d\n", 
			//									base, sent, leftover_len);

                        if (!_drbd_send_page(mdev, pg, base, sent, i == bio->bi_vcnt -1 ? 0 : MSG_MORE))
                                return 0;
                        leftover_len -= sent;
			address += sent;
                        sent = PAGE_SIZE < leftover_len ? PAGE_SIZE : leftover_len;
                        base = 0;
...
/*Ben.Wang*/
	spin_lock_init(&drbdraid.failover_bit_spinlock);
	drbdraid.failover_enable = -1;
/*Ben.Wang*/
...
/*Ben.Wang*/
	unregister_chrdev(DRBD_CTR_MAJOR, DRBD_CTR_NAME);
/*Ben.Wang*/
...
#define DRBD_FALCON_MAGIC		's'
#define DRBD_IOC_MAXNR			16
typedef struct _check_storage {
	int status;
} check_storage;

#define IOCTL_DRBD_CHK_STORAGE            _IOWR(DRBD_FALCON_MAGIC, 1,check_storage)

static ssize_t drbd_ctr_write(struct file *filp, const char *buffer, size_t count, loff_t *ppos)
{
	printk(KERN_ALERT "FALCON: PLACE HOLDER FOR WRITE, NOTHING WILL HAPPEN\n");
	return 1;
}

static int drbd_ctr_open(struct inode *inode, struct file *filp)
{
        int dev = MINOR(inode->i_rdev);

        if (dev != 0)
                return -ENODEV;
	printk(KERN_ALERT "FALCON: DRBD CONTROL DEVICE OPEN\n");
        return 0;
}

static int drbd_ctr_release(struct inode *inode, struct file *filp)
{
	printk(KERN_ALERT "FALCON: DRBD CONTROL DEVICE CLOSE\n");
        return 0;
}

static int drbd_ctr_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	check_storage ck_stroage;

	if (_IOC_TYPE(cmd) != DRBD_FALCON_MAGIC || _IOC_NR(cmd) > DRBD_IOC_MAXNR)
	{
		printk(KERN_ALERT "FALCON: INVALID IOCTL\n");
		return -2010;
	}
	switch (cmd)
	{
		case IOCTL_DRBD_CHK_STORAGE :
			spin_lock(&drbdraid.failover_bit_spinlock);
			ck_stroage.status = drbdraid.failover_enable;
			spin_unlock(&drbdraid.failover_bit_spinlock);
			if (copy_to_user(((void *)arg), (void *)&ck_stroage, _IOC_SIZE(cmd)))
			{
				printk(KERN_ALERT "FALCON: ERROR WHEN COPY TO USER\n");
				return -2011;
			}
			break;
		
		default:
			printk(KERN_ALERT "FALCON: NOT A VALID IOCTL\n");
			break;
	}
...
#else 
static struct file_operations drbd_ctr_fops= {
        .write =        drbd_ctr_write,
        .ioctl =        drbd_ctr_ioctl,
        .open =         drbd_ctr_open,
        .release =      drbd_ctr_release,
};
#endif 
...
/*Ben.Wang*/
	int result = -2010;
	printk(KERN_INFO "falcon_drbd: register the control device\n");
	result = register_chrdev(DRBD_CTR_MAJOR, DRBD_CTR_NAME, &drbd_ctr_fops);
	if (result < 0) 
	{	printk(KERN_ALERT "FALCON DEBUG: fail to register control device, err: %d", result);
		return result;	
	}
/*Ben.Wang*/
