...
static inline int check_health(struct drbd_conf *mdev)
{
	int unconfigured = (mdev->state.conn == C_STANDALONE && mdev->state.disk == D_DISKLESS
                && mdev->state.role == R_SECONDARY);

	int ok_to_failover = (mdev->state.role == R_PRIMARY && mdev->state.disk == D_UP_TO_DATE);

	if (unconfigured || ok_to_failover)
	{
		return 1;
	}
	return 0;
...
{
	unsigned int mdev_minor = -1;
	int healthy_ok_count = 0;
	int num_of_mdev = 0;
	...
	if (minor_table)
	{
		mdev_minor = minor_count;
		while (mdev_minor--)
		{
			mdev = minor_to_mdev(mdev_minor);
			if (NULL != mdev)
			{
				healthy_ok_count += check_health(mdev);
				num_of_mdev ++;
				mdev = NULL;
			} 
		}
	...
	switch (cmd)
	{
		case IOCTL_DRBD_CHK_STORAGE :
			// can failover only if all mdevs are qualified
			ck_stroage.status = (healthy_ok_count ==  num_of_mdev)? 0 : 1;
